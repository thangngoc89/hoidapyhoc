...
;(function(window, jQuery, undefined) {
    ...
    var createLink = function(_this) {

        var pageTitle = window.document.title; //HTML page title
        var pageUrl = window.location.href; //Location of the page
        var pageDescription = window.document.getElementsByTagName("meta")['description']['content'];

        var pageBaseUrl = function () {
            url = window.location.href;
            return url.substring(0, url.indexOf('/', 8));
        };

        var shareName = _this.parent().attr('class').split('-')[1]; //get the first class name of clicked element
        ...
        switch (shareName) //switch to different links based on different social name
        {
            ...
            case 'twitter':
                ...
                break;
            case 'linkedin':
                var openLink = 'http://www.linkedin.com/shareArticle?mini=true&url=' + encodeURIComponent(pageUrl) + '&amp;title=' + encodeURIComponent(pageTitle) + '&amp;summary=' + encodeURIComponent(pageDescription) + '&amp;source=' + encodeURIComponent(pageBaseURL);
                break;
            case 'reddit':
                var openLink = 'http://reddit.com/submit?url=' + encodeURIComponent(pageUrl) + '&amp;title=' + encodeURIComponent(pageTitle);
                break;
            case 'digg':
	...
	jQuery(document).ready(function(){
		...
		jQuery('.rrssb-buttons a.popup').on('click', function(e){
            ...
            var share_link = createLink(_this);

			popupCenter(share_link, _this.find('.rrssb-text').html(), 580, 470);
            e.preventDefault();
